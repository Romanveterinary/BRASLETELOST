...
Window.size = (400, 720)
CONFIG_FILE = "anti_lost_config.json"

class MainScreen(Screen):
    """Головний екран додатка з повзунками та логікою трекінгу"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.monitor_task = None  
        self.disconnect_start_time = None
        self.alarm_start_time = None  # Фіксація часу початку сирени
        
        # Завантажуємо звук тривоги (поклади файл sonar.wav в папку з скриптом)
        ...
        # 3. Повзунок: Сила сигналу (RSSI / Відстань)
        ...
        # 4. Повзунок: Інтервал пінгу (Частота опитування)
        ...
        # 5. Повзунок: Час відповіді (Затримка тривоги)
        ...
        # 6. Новий повзунок: Тривалість звучання тривоги (Твоя ідея!)
        ...
        # Кнопка переходу в налаштування
    ...
    async def ble_monitor_loop(self):
        """Головний фоновий цикл перевірки зв'язку"""
        mac_address = ""
        ...
        while True:
            try:
                ...
                max_alarm_duration = self.duration_slider.value * 60  # Переводимо хвилини в секунди

                device = await BleakScanner.find_device_by_address(mac_address, timeout=1.5)
                current_time = time.time()

                # Перевірка на перевищення ліміту часу звучання сирени
                if self.alarm_start_time and (current_time - self.alarm_start_time >= max_alarm_duration):
                    ...
                    self.status_label.color = (0.7, 0.4, 0.7, 1) # Фіолетовий стан сну
                ...
                if device is None or device.rssi < target_rssi:
                    ...
                    if elapsed >= timeout_limit:
                        ...
                        if self.alarm_start_time is None:
                            self.alarm_start_time = current_time  # Засікаємо, коли загарчала сирена
                        
                        if self.alarm_sound and self.alarm_sound.state == 'stop':
                            self.alarm_sound.play()
                else:
                    # Пристрій повернувся в зону до закінчення таймауту або після
                    if self.alarm_sound and self.alarm_sound.state == 'play':
                        self.alarm_sound.stop()
                    self.disconnect_start_time = None
                    self.alarm_start_time = None
...
class SettingsScreen(Screen):
    """Екран інженерних налаштувань та BLE-радара"""
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        layout = BoxLayout(orientation='vertical', padding=20, spacing=10)
